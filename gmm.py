...
class GMM:
    ...
    def __init__(self, k = 13, eps = 0.0001):
        self.k = k      # 聚类数量
        self.eps = eps  # 终止准则
        
        # 来自拟合/学习的所有数参都保存在一个命名的元组中
        from collections import namedtuple

    def fit_EM(self, X, max_iters = 1000):  #EM期望最大化算法
...
def demo_2d():
    ...
    np.random.seed(3)   #设置相同的seed，每次生成的随机数相同,如果不设置seed，则每次会生成不同的随机数
    m1, cov1 = [9, 8], [[.5, 1], [.25, 1]] ## first gaussian, 均值及协方差矩阵
    data1 = np.random.multivariate_normal(m1, cov1, 90) #从多元正态分布中生成随机抽样，组成一个N维的数组。并返回该数组
...
#   np.savetxt('sample.csv', X, fmt = "%.4f",  delimiter = ",")
    ####
    gmm = GMM(3, 0.000001)   #3个单高斯变量   误差范围
    ...
    plt.title('Log Likelihood -- iteration plot')
